    ...
    <div class="row ">
        <div class="col-md-12">
            <ul class="nav nav-tabs">
                ...
                <li><a href="/partners/temp/setTemp/<?=$id?>"><?=__("filetemp")?></a></li>
            </ul>
            <div class="box">
                <div class="box-body">
                    <h4><i class="fa fa-fw fa-list-ul"></i><?=__('tempfilelist').__('gamen')?></h4>
                    <form action="/partners/temp/lists/<?=$id?>" method="POST">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <?php foreach($list as $key=>$value):?>
                                        <tr >
                                            ...
                                            <td><a href="/partners/temp/download/<?=$value[ 'id' ]?>"><?=h($value[ 'filename' ])?></a></td>
                                            <td><?=h($value[ 'size' ])?></td>
                                            <td><?=h($value[ 'statusDisp' ])?></td>
                                            <td class="text-center">
                                                <a href="/partners/temp/delete/<?=$value[ 'id' ]?>/<?=$id?>" class="btn btn-danger delete-temp" ><?=__("sakujyo")?></a> 
                                            </td>
                                        </tr>
                                    <?php endforeach;?>
                                </table>
